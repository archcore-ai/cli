...
Returns: JSON with path, type, category, title, status, tags (when present), and optionally nearby_documents — paths of other documents in the same directory (capped at 5, sorted alphabetically). Treat nearby_documents as a hint only: review each candidate and call add_relation explicitly when a semantic link exists. Do not link every neighbor by default.`),
		...
		mcp.WithString("content",
			mcp.Description(`Markdown body of the document. RECOMMENDED: omit this parameter to use the auto-generated template for the chosen type (it contains all required sections with guidance placeholders). If providing content manually, omit the top-level heading and follow the template section order for the chosen type. For "spec": write Normative Behavior and Failure Behavior as numbered lines in EARS clause order with BCP 14 modals, e.g. "WHEN <trigger>, the <subject> MUST <response>". One requirement, one modal, one active-voice obligated subject per numbered line (no subjectless passives); commands, requests, and state changes are WHEN triggers, never grammatical subjects; error paths as "IF <condition>, THEN ..."; body <= 80 lines.`),
		),
