...
func TestClaudeCode_WriteMCPConfig_NewFile(t *testing.T) {
	t.Parallel()
	base := t.TempDir()
	agent := ByID(ClaudeCode)
...
func TestClaudeCode_WriteMCPConfig_Idempotent(t *testing.T) {
	t.Parallel()
	base := t.TempDir()
	agent := ByID(ClaudeCode)
...
func TestClaudeCode_Detect_True(t *testing.T) {
	t.Parallel()
	base := t.TempDir()
	...
	agent := ByID(ClaudeCode)
	if !agent.DetectFn(base) {
		t.Error("expected detection when .claude/ exists")
	}
}
...
func TestClaudeCode_Detect_False(t *testing.T) {
	...
	agent := ByID(ClaudeCode)
	if agent.DetectFn(base) {
		t.Error("expected no detection without .claude/")
	}
}
