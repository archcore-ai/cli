...
- **Reporting** — the install confirmation names every file the write touched (`CLAUDE.md and AGENTS.md` for Claude Code). The registry carries this as the optional `ExtraInstructionsPaths` field on `agents.Agent`, so the Claude-specific knowledge stays in `internal/agents`; if a third multi-target agent ever appears, the contract can evolve to a plural `InstructionsPaths` — deliberately not done speculatively for one case.
