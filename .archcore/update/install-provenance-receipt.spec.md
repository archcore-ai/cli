...
This spec defines the **install provenance receipt**: the local file that records which channel put this binary at this path, and the only evidence `archcore update --unattended` accepts as authorization to replace it. Dependents: `@install.sh`, `@install.ps1`, the `update` command (`@cmd/update.go`), and any future package-manager channel, which must not write it.
...
- Reader: `archcore update --unattended`.
...
4. WHEN `archcore update --unattended` reads the receipt, the CLI MUST compare `binary` against the resolved path of the running executable.
...
An implementation is conformant when the installers and the `update` command write the fields in requirements 1–3, unattended replacement consults the receipt per requirements 4–5, and every read failure denies per the fail-closed rule.

Given a binary installed by Homebrew and never touched by `install.sh`, when `archcore update --unattended` runs, then no receipt names that path, and the command exits without contacting the network.
